...
class CameraManager:
    ...
    def setup_camera_bounds(self, tile_map):
        ...
        tile_width = tile_map.width * TILE_SIZE * TILE_SCALING
        tile_height = tile_map.height * TILE_SIZE * TILE_SCALING
        self.camera_bounds = arcade.LRBT(
            self.game_view.window.width / 2.0,
            tile_width - self.game_view.window.width / 2.0,
            self.game_view.window.height / 2.0,
            tile_height,
        )

    def center_camera_to_player(self, delta_time):
        """Center the camera on the player with smooth \
        following."""
