    ...
    def register_component(self, component: Resetable, reset_type: str = "map") -> None:
        ...
        if reset_type == "game":
            if hasattr(component, 'reset_for_game'):
                ...
                print(f"[RESET_COORDINATOR] Warning: {component.__class__.__name__} doesn't implement reset_for_game")
        else:
            if hasattr(component, 'reset_for_map'):
                self.map_resetable_components.append(component)
            else:
                print(f"[RESET_COORDINATOR] Warning: {component.__class__.__name__} doesn't implement reset_for_map")
        
        self.resetable_components.append(component)
        print(f"[RESET_COORDINATOR] Registered {component.__class__.__name__} for {reset_type} reset")
    ...
    def reset_for_map(self) -> None:
        ...
        for component in self.map_resetable_components:
            try:
                ...
                print(f"[RESET_COORDINATOR] Error resetting {component.__class__.__name__}: {e}")
