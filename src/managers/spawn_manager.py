...
class SpawnPoint:
    ...
    def __str__(self):
        return (
            f"SpawnPoint({self.x:.1f}, {self.y:.1f}, "
            f"valid={self.is_valid}, used={self.usage_count})"
        )
...
class SpawnManager:
    ...
    def __init__(self, game_view):
        ...
        if ENABLE_TESTING:
            Debug.track_event(
                "spawn_manager_initialized",
                {"game_view": str(game_view), "spawn_points_count": 0},
            )
    ...
    def load_spawn_points_from_map(self, tile_map) -> List[SpawnPoint]:
        ...
        try:
            ...
            if "Zombie-spawns" in tile_map.object_lists:
                ...
                print(
                    f"[SPAWN_MANAGER] Found {len(spawn_objects)} spawn "
                    f"points in Zombie-spawns layer"
                )

                for spawn_object in spawn_objects:
                    spawn_point = SpawnPoint(
                        x=spawn_object.shape[0], y=spawn_object.shape[1]
                    )
                    spawn_points.append(spawn_point)
                    print(
                        f"[SPAWN_MANAGER] Loaded spawn point at "
                        f"({spawn_point.x:.1f}, {spawn_point.y:.1f})"
                    )
    ...
    def validate_all_spawn_points(self, wall_list) -> List[SpawnPoint]:
        ...
        if ENABLE_TESTING:
            Debug.track_event(
                "spawn_points_validated",
                {
                    "total_spawn_points": len(self.spawn_points),
                    "valid_spawn_points": len(valid_spawn_points),
                    "invalid_spawn_points": len(self.spawn_points)
                    - len(valid_spawn_points),
                },
            )
    ...
    def select_spawn_points(
        self, zombie_count: int, current_time: float
        ...
        for _ in range(min(zombie_count, len(available_spawn_points))):
            ...
            if total_weight > 0:
                ...
                selected_spawn = random.choices(
                    available_spawn_points, weights=normalized_weights
                )[0]
            else:
    ...
    def create_zombie(self, x: float, y: float):
        ...
        print(
            f"[SPAWN_MANAGER] Added zombie to scene at "
            f"({zombie.center_x:.1f}, {zombie.center_y:.1f})"
        )

        if ENABLE_TESTING:
            Debug.track_event(
                "enemy_spawned_at_position",
                {"x": int(x), "y": int(y), "enemy_type": "Army_zombie"},
            )
