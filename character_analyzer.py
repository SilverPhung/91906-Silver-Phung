...
def analyze_player_directory(base_dir: str) -> Dict[str, Any]:
    ...
    character_folders = [f for f in os.listdir(base_dir) 
    ...
    for character in character_folders:
        ...
        animation_folders = [f for f in os.listdir(character_path) 
        ...
        for animation in animation_folders:
            ...
            if png_files:
                ...
                config[character][animation] = {
                    ...
                    "notes": "Adjust anchor_x and anchor_y to match character's center of mass for this animation"
                }
...
def analyze_zombie_directory(base_dir: str) -> Dict[str, Any]:
    ...
    zombie_folders = [f for f in os.listdir(base_dir) 
                     if os.path.isdir(os.path.join(base_dir, f))]
    ...
    for zombie_type in zombie_folders:
        ...
        animation_folders = [f for f in os.listdir(zombie_path) 
    ...
    sample = {
        "Girl": {
            "Walk_gun": {
                ...
                "notes": "Set anchor_x and anchor_y to the character's center of mass for proper rotation"
            },
            "Riffle": {
                "anchor_x": 64,
                "anchor_y": 80,
                ...
                "notes": "Each animation folder has its own anchor point settings"
            }
        }
    }
...
def get_user_input():
    ...
    player_input = input(f"Enter player assets directory path (default: {DEFAULT_PLAYER_ASSETS_DIR}): ").strip()
    player_dir = player_input if player_input else DEFAULT_PLAYER_ASSETS_DIR
    
    # Get zombie assets directory
    zombie_input = input(f"Enter zombie assets directory path (default: {DEFAULT_ZOMBIE_ASSETS_DIR}): ").strip()
...
def main():
    ...
    player_dir, zombie_dir = get_user_input()
    ...
    if player_config:
        ...
        print(f"Generated {player_config_path} with {len(player_config)} characters")
    ...
    if zombie_config:
        ...
        print(f"Generated {zombie_config_path} with {len(zombie_config)} zombie types")
    ...
    print("\nNext steps:")
    print(f"1. Open {os.path.join(OUTPUT_DIR, 'players_config.json')} and {os.path.join(OUTPUT_DIR, 'zombies_config.json')}")
    print("2. For each animation folder, adjust anchor_x and anchor_y to match the character's center of mass")
